...
Кстати забыл рассказать о замечательной дополнительной tool netlab которая позволяет не путаться в том что и как соединено - graphite. Она отражает как соединены интерфейсы, а также позволяет подключаться на устройства по ssh, причем можно одновременно висеть на нескольких устройствах, что на мой взгляд очень удобно, если не любишь tmux. ![graphite](graphite.png)

Теперь немного поснифаем трафик, чтобы посмотреть что происходит на leaf-4

Маршруты типа 5 отдаваемые на spine ![vni10000](l4-vni10000.png) ![vni10001](l4-vni10001.png)

Маршруты типа 3 ![vni10100](l4-vni10100.png) ![vni10101](l4-vni10101.png)

Посмотрим в icmp на l4 ![h1-h6](h1-h6.png) видим что пакет пришел из фабрики по vni 10000(vrf red type 5) а ответ ушел по vni 10000 (vlan 1000 type 3) ![h1-h6reply](h1-h6reply.png) Что то я немного приплутал.

Проще всего посмотреть круговорот трафика в фабрике оказалось на l3 пропинговав хосты подключенные к нему в разных vrf ![h3-h4](h3-h4.png) Так на 1 запрос от h3 улетает в фабрику, на 2 этот же запрос возвращается из фабрики, на 3 ответ от h4 улетает в фабрику, на 4 ответ прилетает от фабрики. В обратную сторону по ipv6 ![h4-h3v6](h4-h3v6.png)
...
![Leaf-1](l1.cfg) ![Leaf-2](l2.cfg) ![Leaf-3](l3.cfg) ![Leaf-4](l4.cfg) ![R-1](r1.cfg) ![Spine-1](s1.cfg) ![Spine-2](s2.cfg)
